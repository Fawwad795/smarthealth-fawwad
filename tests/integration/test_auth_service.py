...
def test_register_patient_rejects_a_duplicate_email_case_insensitively(
    ...
    register_patient(
        db_session,
        RegisterRequest(
            email="ayesha@example.com", password="a real password", dob="1990-05-14"
        ),
    )
...
def test_login_returns_a_token_for_the_right_user(db_session: Session) -> None:
    ...
    token = login(
        db_session,
        LoginRequest(
            email="ayesha@example.com", password="correct horse battery staple"
        ),
    )
...
def test_login_rejects_wrong_password_and_missing_email_identically(
    ...
    with pytest.raises(AppError) as no_such_email:
        login(
            db_session, LoginRequest(email="nobody@example.com", password="whatever123")
        )
    ...
    assert (
        wrong_password.value.code == no_such_email.value.code == "INVALID_CREDENTIALS"
    )
